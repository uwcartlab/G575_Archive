...
#prep to data
geojson_filename = paste(trimws(geojson_input_dir),"/Hawaii_Volcanoes_National_Park.GeoJSON",sep="")
print(geojson_filename)
csv_filename = paste(trimws(flickr_input_dir),"/Hawaii_Volcanoes_National_Park.csv",sep="")
print(csv_filename)
buffer_filename = paste(trimws(nps_dir),"/nps_plus_buffer.shp",sep="")
print(buffer_filename)
csv_filename_out = paste(trimws(output_dir),"/Hawaii_Volcanoes_National_Park.csv",sep="")
geojson_filename_out = paste(trimws(output_dir),"/Hawaii_Volcanoes_National_Park.GeoJSON",sep="")
print(geojson_filename_out)

bb <- readOGR(dsn=buffer_filename)
bb2 <- spTransform(bb, CRS(wgs84))  
clip_bb <- subset(bb2, UNIT_CODE=="HAVO")
#"ACAD", "ARCH", "BADL", "BIBE", 
# "BRCA", "CANY", "CARE", "CAVE", 
#"CHIS", "CRLA", "CUVA", "DENA",
#"DEVA", "EVER", "GLAC", "GLBA",
...
aea_proj_HAVO <- paste("+proj=aea +lat_1=",trimws(lat_1)," +lat_2=",trimws(lat_2)," +lat_0=",trimws(lat_0),
...
r2 <- projectRaster(r, crs=aea_proj_HAVO, res=15)


####################
#6. Calculate distance to road within buffer
#proj roads
HAVO <- readOGR(geojson_filename)
HAVO_roads <- spTransform(HAVO, CRS(aea_proj_HAVO))
#make road raster
roads.r <- rasterize(HAVO_roads, r2, field=1)
...
#coordinates
#id
HAVO_csv<-read.csv(csv_filename, head=TRUE, sep=",",stringsAsFactors=FALSE)

##############
#8. extract value to point
#make spdf
HAVO_spdf <- SpatialPointsDataFrame(HAVO_csv[,2:3],
                                    HAVO_csv,    #the R object to convert
                                    proj4string = CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))   # assign a CRS 
HAVO_flickr <- spTransform(HAVO_spdf, CRS(aea_proj_HAVO))

#value to point                          
distance <- raster::extract(roaddist.r, HAVO_flickr)
HAVO_distance <- unlist(distance)
HAVO$distance2 <-as.data.frame(HAVO$distance)
#HAVO_distance2 <- as.data.frame(HAVO_distance)
HAVO_distance3$distance <-HAVO_distance2$HAVO_distance
HAVO_distance4 <- cbind(HAVO_csv,HAVO_distance3)
#myvars <- c("id", "long", "lat", "distance")
#HAVO_distance2 <- HAVO_distance2[myvars]
##############
#9. write files
#write csv
write.csv(HAVO_distance4, file = csv_filename_out,row.names=FALSE)
