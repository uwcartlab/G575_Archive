...
#prep to data
geojson_filename = paste(trimws(geojson_input_dir),"/Rocky_Mountain_National_Park.GeoJSON",sep="")
print(geojson_filename)
csv_filename = paste(trimws(flickr_input_dir),"/Rocky_Mountain_National_Park.csv",sep="")
print(csv_filename)
buffer_filename = paste(trimws(nps_dir),"/nps_plus_buffer.shp",sep="")
print(buffer_filename)
csv_filename_out = paste(trimws(output_dir),"/Rocky_Mountain_National_Park.csv",sep="")
geojson_filename_out = paste(trimws(output_dir),"/Rocky_Mountain_National_Park.GeoJSON",sep="")
print(geojson_filename_out)
bb <- readOGR(dsn=buffer_filename)
bb2 <- spTransform(bb, CRS(wgs84))  
clip_bb <- subset(bb2, UNIT_CODE=="ROMO")
#"ACAD", "ARCH", "BADL", "BIBE", 
# "BRCA", "CANY", "CARE", "CAVE", 
#"CHIS", "CRLA", "CUVA", "DENA",
#"DEVA", "EVER", "GLAC", "GLBA",
...
aea_proj_ROMO <- paste("+proj=aea +lat_1=",trimws(lat_1)," +lat_2=",trimws(lat_2)," +lat_0=",trimws(lat_0),
...
r2 <- projectRaster(r, crs=aea_proj_ROMO, res=15)


####################
#6. Calculate distance to road within buffer
#proj roads
ROMO <- readOGR(geojson_filename)
ROMO_roads <- spTransform(ROMO, CRS(aea_proj_ROMO))
#make road raster
roads.r <- rasterize(ROMO_roads, r2, field=1)
...
#coordinates
#id
ROMO_csv<-read.csv(csv_filename, head=TRUE, sep=",",stringsAsFactors=FALSE)

##############
#8. extract value to point
#make spdf
ROMO_spdf <- SpatialPointsDataFrame(ROMO_csv[,2:3],
                                    ROMO_csv,    #the R object to convert
                                    proj4string = CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))   # assign a CRS 
ROMO_flickr <- spTransform(ROMO_spdf, CRS(aea_proj_ROMO))

#value to point                          
distance <- raster::extract(roaddist.r, ROMO_flickr)
ROMO_distance <- unlist(distance)
ROMO$distance2 <-as.data.frame(ROMO$distance)
#HALE_distance2 <- as.data.frame(HALE_distance)
ROMO_distance3$distance <-ROMO_distance2$ROMO_distance
ROMO_distance4 <- cbind(ROMO_csv,ROMO_distance3)
#myvars <- c("id", "long", "lat", "distance")
#HALE_distance2 <- HALE_distance2[myvars]
##############
#9. write files
#write csv
write.csv(ROMO_distance4, file = csv_filename_out,row.names=FALSE)
