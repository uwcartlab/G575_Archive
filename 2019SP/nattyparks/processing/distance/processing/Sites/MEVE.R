...
#prep to data
geojson_filename = paste(trimws(geojson_input_dir),"/Mesa_Verde_National_Park.GeoJSON",sep="")
print(geojson_filename)
csv_filename = paste(trimws(flickr_input_dir),"/Mesa_Verde_National_Park.csv",sep="")
print(csv_filename)
buffer_filename = paste(trimws(nps_dir),"/nps_plus_buffer.shp",sep="")
print(buffer_filename)
csv_filename_out = paste(trimws(output_dir),"/Mesa_Verde_National_Park.csv",sep="")
geojson_filename_out = paste(trimws(output_dir),"/Mesa_Verde_National_Park.GeoJSON",sep="")
print(geojson_filename_out)
bb <- readOGR(dsn=buffer_filename)
bb2 <- spTransform(bb, CRS(wgs84))  
clip_bb <- subset(bb2, UNIT_CODE=="MEVE")
#"ACAD", "ARCH", "BADL", "BIBE", 
# "BRCA", "CANY", "CARE", "CAVE", 
#"CHIS", "CRLA", "CUVA", "DENA",
#"DEVA", "EVER", "GLAC", "GLBA",
...
aea_proj_MEVE <- paste("+proj=aea +lat_1=",trimws(lat_1)," +lat_2=",trimws(lat_2)," +lat_0=",trimws(lat_0),
...
r2 <- projectRaster(r, crs=aea_proj_MEVE, res=15)


####################
#6. Calculate distance to road within buffer
#proj roads
MEVE <- readOGR(geojson_filename)
MEVE_roads <- spTransform(MEVE, CRS(aea_proj_MEVE))
#make road raster
roads.r <- rasterize(MEVE_roads, r2, field=1)
...
#coordinates
#id
MEVE_csv<-read.csv(csv_filename, head=TRUE, sep=",",stringsAsFactors=FALSE)

##############
#8. extract value to point
#make spdf
MEVE_spdf <- SpatialPointsDataFrame(MEVE_csv[,2:3],
                                    MEVE_csv,    #the R object to convert
                                    proj4string = CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))   # assign a CRS 
MEVE_flickr <- spTransform(MEVE_spdf, CRS(aea_proj_MEVE))

#value to point                          
distance <- raster::extract(roaddist.r, MEVE_flickr)
MEVE_distance <- unlist(distance)
MEVE$distance2 <-as.data.frame(MEVE$distance)
#MEVE_distance2 <- as.data.frame(MEVE_distance)
MEVE_distance3$distance <-MEVE_distance2$MEVE_distance
MEVE_distance4 <- cbind(MEVE_csv,MEVE_distance3)
#myvars <- c("id", "long", "lat", "distance")
#MEVE_distance2 <- MEVE_distance2[myvars]
##############
#9. write files
#write csv
write.csv(MEVE_distance4, file = csv_filename_out,row.names=FALSE)
